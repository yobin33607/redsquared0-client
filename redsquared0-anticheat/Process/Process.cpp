...
FARPROC Process::_GetProcAddress(__in const PCSTR Module, __in const LPCSTR lpProcName)
{
    ...
    if (Module != nullptr)
    {
        int requiredSize = MultiByteToWideChar(CP_UTF8, 0, Module, -1, NULL, 0);
        if (requiredSize > 0)
        {
            moduleNameW.resize(requiredSize);
            MultiByteToWideChar(CP_UTF8, 0, Module, -1, &moduleNameW[0], requiredSize);
            if (!moduleNameW.empty() && moduleNameW.back() == L'\0')
            {
                moduleNameW.pop_back();
            }
        }
    }
    ...
    {
        ModuleBase = (uintptr_t)Process::GetModuleHandle_Ldr(moduleNameW.c_str());
    }

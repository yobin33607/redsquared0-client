 ...
 * @details This routine has O(n^2) execution time, and should go in its own thread or future
 * @return list of hooked iat entries
 *
 * @usage
 *  std::list<ProcessData::ImportFunction> hookedIATEntries = Integrity::FetchHookedIATEntries();
 */
std::list<ProcessData::ImportFunction> Detections::FetchHookedIATEntries()
{
    bool isIATHooked = false;
                ...
                if (QueryWorkingSetEx(GetCurrentProcess(), &wsInfo, sizeof(wsInfo)))
                {
                    if (wsInfo.VirtualAttributes.Valid)
                    {
                        if (!wsInfo.VirtualAttributes.Shared)  // If not shared, it's likely private
                        {
                            ...
                            const SIZE_T scanSize = min< SIZE_T >(mbi.RegionSize, 4096);
                            std::vector<unsigned char> bufferPossibleMappedSection(scanSize, 0);

                            // scan the first committed page for meaningful bytes rather than assuming a fixed offset
                            ...
                            if (ReadProcessMemory(GetCurrentProcess(), (LPCVOID)possibleTextSectionAddress, bufferPossibleMappedSection.data(), scanSize, NULL))
                            {
                                for (SIZE_T i = 0; i + 4 < scanSize; i++) // many manual mappers erase headers by replacing them with all 00's
                                {

...
bool Preventions::StopAPCInjection()
{
    ...
    const int Ordinal = 8;
    FARPROC pApcTarget = GetProcAddress(ntdll, "NtQueueApcThread");
    if (pApcTarget == NULL)
    {
        pApcTarget = GetProcAddress(ntdll, MAKEINTRESOURCEA(Ordinal));
    }

    UINT64 Oridinal8 = (UINT64)pApcTarget;

    if (!Oridinal8)
    {
        Logger::logf(Err, "Failed to fetch ntdll APC queue target address @ StopAPCInjection");
        return false;
    }

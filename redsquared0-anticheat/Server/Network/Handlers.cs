...
namespace UACServer.Network
{
    internal class Handlers
    {
        ...
        public static void HandleClientFlaggedCheater(AntiCheatClient c, DetectionFlags flag)
        {
            if (AnticheatServer.Detections.TryGetValue(flag, out string reason))
            {
                Logger.Log("UACServer.log", $"[DETECTION] Client #{c.id} was flagged for {reason}");
            }
            else
            {
                Logger.Log("UACServer.log", $"[DETECTION] Client #{c.id} was flagged for unknown or unmapped detection flag ({flag})");
            }

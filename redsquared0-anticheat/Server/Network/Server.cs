    ...
    class AnticheatServer
    {
        ...
        private bool HandlePacket(AntiCheatClient c, byte[] buffer, int length)
        {
            ...
            switch ((Opcodes.CS)opcode)
            {
                ...
                {
                    ushort memorySize = p.ReadUShort();
                    byte[] memoryBytes = p.ReadBytes(memorySize);

                    string bytesHex = BitConverter.ToString(memoryBytes).Replace("-", " ");
                    Logger.Log("UACServer.log", $"Received memory query response from client #{c.id}: {memorySize} bytes. Data preview: {bytesHex.Substring(0, Math.Min(bytesHex.Length, 128))}");
                }
                break;
